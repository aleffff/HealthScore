...
public sealed class AnalyticsFilterTests
{
    ...
    {
        var filter = new AnalyticsFilter(" Farma ", " ", "Suporte", " Farma Big ", "invalid");

        Assert.Equal("Farma", filter.Brand);
        Assert.Null(filter.Product);
        Assert.Equal("Suporte", filter.Scope);
        Assert.Equal("Farma Big", filter.BusinessUnit);
    ...
    {
        Assert.Equal(mode, new AnalyticsFilter(null, null, null, null, mode).Issue);
    }
}
