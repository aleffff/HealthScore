  ...
  <div class="audit-shell">
    ...
    <main>
      ...
      <template v-if="audit">
        ...
        <section class="audit-card">
          <div class="audit-heading"><div><p class="audit-eyebrow">BASE DO CÁLCULO</p><h3>Chamados utilizados</h3></div><div class="case-tools"><form @submit.prevent="casesPage = 1; loadCases()"><input v-model="caseSearch" placeholder="Número do chamado" /><button>Buscar</button></form><label class="anomaly-toggle"><input v-model="anomaliesOnly" type="checkbox" @change="toggleAnomalies" /> Mostrar apenas anomalias</label></div></div>
          <div class="audit-table-wrap"><table><thead><tr><th>Chamado</th><th>Conta</th><th>Criação</th><th>Status / prioridade</th><th>SLA</th><th>FCR</th><th>Issue</th><th>Produto / escopo</th><th>Taxonomia</th></tr></thead>
            <tbody><tr v-for="item in cases" :key="item.salesforceId"><td><strong>{{ item.caseNumber }}</strong><small>{{ item.salesforceId }}</small></td><td>{{ item.accountSalesforceId || '—' }}</td><td>{{ date(item.salesforceCreatedAt) }}</td><td>{{ item.status || '—' }}<small>{{ item.priority || 'sem prioridade' }}</small></td><td>{{ item.slaViolated === null ? 'Ausente' : item.slaViolated ? 'Violado' : 'OK' }}</td><td>{{ item.firstContactResolution === null ? 'Ausente' : item.firstContactResolution ? 'Sim' : 'Não' }}</td><td>{{ item.jiraIssueCode || '—' }}</td><td>{{ item.product || '—' }}<small>{{ item.openingVertical || '—' }}</small></td><td>{{ item.taxonomy || '—' }}</td></tr></tbody>
          </table></div>
